...
static void window_create(monitor_t * m)
{
	...
	int flag = SDL_WINDOW_SKIP_TASKBAR | SDL_WINDOW_BORDERLESS; // SDL_WINDOW_HIDDEN SDL_WINDOW_SKIP_TASKBAR
#if SDL_FULLSCREEN
    flag |= SDL_WINDOW_FULLSCREEN;
#endif

    m->window = SDL_CreateWindow("TFT Simulator 1",
                              SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED,
                              SDL_HOR_RES * SDL_ZOOM, SDL_VER_RES * SDL_ZOOM, flag);       /*last param. SDL_WINDOW_BORDERLESS to hide borders*/
	
	SDL_SetWindowTitle(m->window, "Lucas");
